...
resource "aws_cloudfront_distribution" "tneu_scores" {
  ...
  origin {
    domain_name = "${aws_route53_record.tneu_edu_ua.name}"
    origin_id   = "News-${aws_route53_record.tneu_edu_ua.name}"
  ...
  cache_behavior {
    ...
    target_origin_id = "News-${aws_route53_record.tneu_edu_ua.name}"
    path_pattern     = "/news/*.html"
